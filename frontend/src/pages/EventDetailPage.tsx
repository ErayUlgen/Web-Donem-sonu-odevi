...
import { Calendar, MapPin, ArrowLeft, Tag, Share2, Ticket, MessageCircle, Send, User, Trash2 } from 'lucide-react';
import { format } from 'date-fns';
import { tr } from 'date-fns/locale';
import { toast } from 'react-hot-toast';
import { useAuth } from '../context/AuthContext'; // Giriş kontrolü için
...
export default function EventDetailPage() {
  ...
  const { isAuthenticated, token, user } = useAuth(); // Auth bilgilerini al (user bilgisini token'dan çözmemiz gerekebilir ama şimdilik basit tutalım)
                    ...
                    {comments.length === 0 ? (
                        ...
                        comments.map((comment) => (
                            <div key={comment.id} style={{ display: 'flex', gap: '15px', paddingBottom: '20px', borderBottom: '1px solid #e2e8f0' }}>
                                ...
                                {/* Admin veya Yorum Sahibi Silebilir (Şimdilik sadece giriş yapan herkes butonu görüyor, backend zaten koruyor) */}
                                {isAuthenticated && (
